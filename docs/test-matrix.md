...
| **Manifest & Request Materialization** | Native manifest projection (`prm.export-manifest.json`), verification request derivation (`prm.verification.json`), reference traversal request (`prm.reference-traversal-request.json`) | `go test ./internal/engine/cadruntime/... ./internal/engine/adapter/freecad/...` | Deterministic request JSON materialization |
